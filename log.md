...
*   **Ephemeral Leaf Telemetry & Pure Root Persona ([ADR 003](../decisions/003-ephemeral-leaf-telemetry-pure-root-persona.md))**: Decoupled transient workspace environmental data from the Genesis DAG node. Root nodes (`RoleSystem`) in SQLite now persist 100% pure persona. Dynamic workspace telemetry (shallow directory tree skipping hidden files, `index.md` snippet, and condensed tool guidelines) is projected ephemerally onto the active leaf user turn during `BuildLLMContext()` in [service.go](internal/engine/service.go). Guarantees 100% prefix KV-cache reuse for the persona, enables cross-directory session portability ("waking the train"), and cleans visual cards across TUI and companion clients.
