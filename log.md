...
*   **Global Real-Time Event Bus & Multi-Client Live Sync (`GET /api/v1/events`)**: Implemented an in-memory Pub/Sub `EventBus` in [bus.go](internal/server/bus.go) and SSE streaming endpoint `GET /api/v1/events` in [server.go](internal/server/server.go) broadcasting `node_saved`, `branch_pruned`, and `branch_compacted` mutations. Added background event stream subscription in [events.go](internal/tui/events.go) and [tui.go](internal/tui/tui.go) enabling instant live synchronization across multiple connected TUI terminals ("Two terminals, one brain").
