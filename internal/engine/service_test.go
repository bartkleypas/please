...
func TestBuildLLMContext_ImageFallbackFlow(t *testing.T) {
	...
	mgr.AttachImages(node, []string{img1, img2})
	...
	if msgV.Images[0] != img1 || msgV.Images[1] != img2 {
		t.Errorf("unexpected image paths: %+v", msgV.Images)
	}
	if !strings.Contains(msgV.Content, filepath.Base(img1)) {
		t.Errorf("expected vision message content to contain image filename, got: %s", msgV.Content)
	}
	...
	if !strings.Contains(msgT.Content, fmt.Sprintf("[Attached Image: %s]", filepath.Base(img1))) {
		t.Errorf("expected fallback content to contain attached image indicator, got: %s", msgT.Content)
	}
}
...
func TestBuildLLMContext_PureRootAndUnbumperedUserTurns(t *testing.T) {
	...
	// 3. Verify context structure (ADR 003 Refined: Zero Ventriloquism):
	// Message 0: Pure root persona
	if messages[0].Content != rootPrompt {
		t.Errorf("expected root message in context to be pure persona %q, got: %q", rootPrompt, messages[0].Content)
	}

	// Message 1 (Historical User Turn 1): 100% untouched human words
	if messages[1].Content != "Turn 1 prompt" {
		t.Errorf("expected historical user turn 1 to be pure prompt %q, got: %q", "Turn 1 prompt", messages[1].Content)
	}
...
func TestParseCompactionOutput(t *testing.T) {
	t.Run("Standard dual yield output", func(t *testing.T) {
		raw := `SUMMARY:
We refactored the database schema to enable WAL mode and updated hermetic tests.

MEMORIES:
- key: sqlite_wal_mode | category: architecture | content: SQLite database uses WAL mode for concurrent reads.
- key: test_runner | category: workflow | content: Hermetic tests are run using make test.
- key: code_formatting | category: convention | content: Code must strictly pass gofumpt formatting.`

		summary, memories := parseCompactionOutput(raw, "node-xyz")
		if summary != "We refactored the database schema to enable WAL mode and updated hermetic tests." {
		...
		m1 := memories[0]
		if m1.Key != "sqlite_wal_mode" || m1.Category != storage.CategoryArchitecture || m1.Content != "SQLite database uses WAL mode for concurrent reads." {
			t.Errorf("unexpected m1: %+v", m1)
		}
		if m1.Scope != storage.ScopeWorkspace || m1.SourceNodeID != "node-xyz" || m1.Confidence != 0.9 {
			t.Errorf("unexpected m1 metadata: %+v", m1)
		}

		m2 := memories[1]
		if m2.Key != "test_runner" || m2.Category != storage.CategoryWorkflow || m2.Content != "Hermetic tests are run using make test." {
			t.Errorf("unexpected m2: %+v", m2)
		}

		m3 := memories[2]
		if m3.Key != "code_formatting" || m3.Category != storage.CategoryConstraint || m3.Content != "Code must strictly pass gofumpt formatting." {
			t.Errorf("unexpected m3: %+v", m3)
		}
	})

	t.Run("Markdown bolding and numbered bullets", func(t *testing.T) {
		raw := `**SUMMARY:**
Architectural decision made on session routing.

**MEMORIES:**
1. **key:** session_routing | **category:** architecture | **content:** Sessions are tracked by UUIDv7 heads.
2) **key:** editor_pref | **category:** preference | **content:** User prefers vim navigation.`

		summary, memories := parseCompactionOutput(raw, "source-node-1")
		if summary != "Architectural decision made on session routing." {
			t.Fatalf("unexpected summary: %q", summary)
		}
		if len(memories) != 2 {
			t.Fatalf("expected 2 memories, got %d", len(memories))
		}
		if memories[0].Key != "session_routing" || memories[0].Category != storage.CategoryArchitecture {
			t.Errorf("unexpected mem 0: %+v", memories[0])
		}
		if memories[1].Key != "editor_pref" || memories[1].Category != storage.CategoryPreference {
			t.Errorf("unexpected mem 1: %+v", memories[1])
		}
	})

	t.Run("Explicit none memories", func(t *testing.T) {
		raw := `SUMMARY:
Routine greeting and status check.

MEMORIES:
none`
		summary, memories := parseCompactionOutput(raw, "source-node-2")
		if summary != "Routine greeting and status check." {
			t.Fatalf("unexpected summary: %q", summary)
		}
		if len(memories) != 0 {
	...
	t.Run("Legacy plain text without headings", func(t *testing.T) {
		raw := "This is a plain narrative summary generated by a model without headers."
		summary, memories := parseCompactionOutput(raw, "source-node-3")
		if summary != raw {
			t.Fatalf("expected raw text as fallback summary, got: %q", summary)
		}
		if len(memories) != 0 {
			t.Fatalf("expected 0 memories, got %d", len(memories))
		}
...
func TestCompactRangeWithDirective_MemoryHarvesting(t *testing.T) {
	memStore := &MockMemoryStorage{
		memories: make(map[string]*storage.Memory),
	}
...
MEMORIES:
- key: sqlite_wal | category: architecture | content: SQLite configured with WAL mode.
- key: hermetic_tests | category: workflow | content: Tests are hermetic and run via make test.`,
	}
	...
	// 1. Verify Supernode content contains trajectory and clean summary paragraph
	if !strings.Contains(superNode.Content, "🎯 Trajectory: 🛠️💻 ➔ 🧪⚡") {
		t.Errorf("expected trajectory header, got: %s", superNode.Content)
	}
	if !strings.Contains(superNode.Content, "Configured SQLite WAL journal mode and created hermetic test suite.") {
		t.Errorf("expected summary paragraph, got: %s", superNode.Content)
	}
	// Supernode content should NOT contain the raw MEMORIES block
	if strings.Contains(superNode.Content, "MEMORIES:") {
...
func TestCompactRangeWithDirective_LegacyFallback(t *testing.T) {
	memStore := &MockMemoryStorage{
		memories: make(map[string]*storage.Memory),
	}
	mgr := NewManager(graph.NewGraph(), memStore)
	...
	if !strings.Contains(superNode.Content, "Legacy milestone summary without any sections.") {
		t.Errorf("expected legacy summary in supernode content: %s", superNode.Content)
	}
	if count, ok := superNode.Metadata["memories_harvested"]; ok && count != "" && count != "0" {
		t.Errorf("expected no harvested memories metadata, got %q", count)
	}
	if len(memStore.memories) != 0 {
